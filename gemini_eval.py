...
def _build_prompt(question_text: str, question_type: str, answer_text: str = None,
                  options: dict = None, correct_option: str = None, ideal_answer: str = None) -> str:
    details = []
    if options:
...
STUDENT'S ANSWER: {answer_text or "(provided as image/audio — see attached)"}
...
## 💡 Correct Answer
Provide the complete, ideal correct answer to the question. Use the IDEAL CORRECT ANSWER/RUBRIC if provided as your primary reference.
...
def evaluate_text_answer(question_text: str, question_type: str, answer_text: str,
    ...
    try:
        ...
        prompt = _build_prompt(question_text, question_type, answer_text, options, correct_option, ideal_answer)
...
def evaluate_image_answer(question_text: str, question_type: str, image_bytes: bytes, mime_type: str = "image/png",
    ...
    try:
        model = genai.GenerativeModel(MODEL)
        prompt = _build_prompt(question_text, question_type, answer_text=None, options=options, correct_option=correct_option, ideal_answer=ideal_answer)
...
def evaluate_answer(question_text: str, question_type: str,
                    ...
                    options: dict = None, correct_option: str = None, ideal_answer: str = None) -> str:
    if image_bytes:
        return evaluate_image_answer(question_text, question_type, image_bytes, image_mime, options, correct_option, ideal_answer)
    elif audio_bytes:
        return evaluate_audio_answer(question_text, question_type, audio_bytes, audio_mime, options, correct_option, ideal_answer)
    else:
        return evaluate_text_answer(question_text, question_type, answer_text or "", options, correct_option, ideal_answer)
